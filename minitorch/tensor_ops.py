...
from minitorch.tensor_data import MAX_DIMS
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        for ordinal in range(len(out)):
            # Step 1: get index from out storage's ordinal
            to_index(ordinal, out_shape, out_index)

            # Step 2: broadcast out index to in index
            broadcast_index(out_index, out_shape, in_shape, in_index)

            # Step 3: get data from in_storage
            data = in_storage[index_to_position(in_index, in_strides)]

            # Step 4: map data
            mapped_data = fn(data)

            # Step 5: store mapped data in out
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for ordinal in range(len(out)):
            # Step 1: get out-index from ordinal
            to_index(ordinal, out_shape, out_index)

            # Step 2: broadcast out-index to a-index and b-index
            broadcast_index(out_index, out_shape, a_shape, a_index)
            broadcast_index(out_index, out_shape, b_shape, b_index)

            # Step 3: get data from a-storage and b-storage
            ...
            # Step 5: store data in out-storage
            out[index_to_position(out_index, out_strides)] = mapped_data
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        for ordinal in range(len(out)):
            ...
            for a_dimi in range(a_shape[reduce_dim]):
                a_index = out_index.copy()
                a_index[reduce_dim] = a_dimi
