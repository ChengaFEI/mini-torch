...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    output = tiled_input.mean(dim=4)

    return output.view(batch, channel, tiled_height, tiled_width)
...
def maxpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    """
    batch, channel, _, _ = input.shape
    tiled_input, tiled_height, tiled_weight = tile(input, kernel)
    output = max(tiled_input, 4)
    return output.view(batch, channel, tiled_height, tiled_weight)
